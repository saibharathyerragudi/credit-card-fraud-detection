...
![Credit card fraud detection workflow](screenshots/fraud-detection-workflow.png)
...
Result: schema tests passed.

The full training and prediction workflow requires the excluded Kaggle dataset at `data/creditcard.csv`. The Flask API also requires a trained model artifact at `models/fraud_detection_pipeline.joblib`, which is generated by `scripts/train_baseline.py`.
